...
class account_partner_ledger(osv.osv_memory):
    ...
    """
    _name = 'account.partner.ledger_with_origin'
    ...
    _columns = {
        ...
        'journal_ids': fields.many2many('account.journal', 'account_partner_ledger_with_origin_journal_rel',
                                        'account_id', 'journal_id', 'Journals', required=True),
    }
    ...
    def onchange_filter(self, cr, uid, ids, filter='filter_no', fiscalyear_id=False, context=None):
        res = super(account_partner_ledger, self).onchange_filter(cr, uid, ids, filter=filter,
        ...
        if filter in ['filter_no', 'unreconciled']:
            ...
            res['value'].update({'initial_balance': False, 'period_from': False, 'period_to': False, 'date_from': False,
                                 'date_to': False})
        return res
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        ...
        if data['form'].get('page_split') is True:
            return self.pool['report'].get_action(cr, uid, [], 'account_report_partner_ledger.report_partnerledgerother', data=data,
                                                  context=context)
        return self.pool['report'].get_action(cr, uid, [],
                                              'account_report_partner_ledger.report_partnerledger',
