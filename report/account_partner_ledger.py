...
class third_party_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self, partner):
        ...
        self.cr.execute(
            "SELECT l.id, l.date, j.code, acc.code as a_code, acc.name as a_name, l.ref,inv.origin as inv_origin, m.name as move_name, l.name, l.debit, l.credit, l.amount_currency,l.currency_id, c.symbol AS currency_code " \
            ...
            "LEFT JOIN account_invoice inv ON (inv.number=l.ref)" \
    ...
    def _sum_currency_amount_account(self, account, form):
        ...
        self.cr.execute(
            "SELECT sum(aml.amount_currency) FROM account_move_line as aml,res_currency as rc WHERE aml.currency_id = rc.id AND aml.account_id= %s ",
            (account.id,))
...
class report_partnerledger(osv.AbstractModel):
    _name = 'report.account_report_partner_ledger.report_partnerledger'
    _inherit = 'report.abstract_report'
    _template = 'account_report_partner_ledger.report_partnerledger'
    _wrapped_report_class = third_party_ledger


class report_partnerledgerother(osv.AbstractModel):
    _name = 'report.account_report_partner_ledger.report_partnerledgerother'
    _inherit = 'report.abstract_report'
    _template = 'account_report_partner_ledger.report_partnerledgerother'
    _wrapped_report_class = third_party_ledger
